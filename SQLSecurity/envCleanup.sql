...
BEGIN
	ALTER LOGIN Rona REVOKE ALTER ANY LOGIN;
...
(
    SELECT *
    FROM sys.server_principals
    WHERE [name] = 'crole_Troubleshooting'
)
...
(
    SELECT *
    FROM sys.database_principals
    WHERE [name] = 'crole_StandardReports'
)
...
(
    SELECT *
    FROM sys.database_principals
    WHERE [name] = 'crole_SensitiveReports'
)
